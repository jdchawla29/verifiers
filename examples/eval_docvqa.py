...
def main():
    ...
    logger.info(
        f"Answer Accuracy - Mean: {answer_stats['mean']:.3f}, Min: {answer_stats['min']:.3f}, Max: {answer_stats['max']:.3f}"
    )
    logger.info(
        f"Format Score - Mean: {format_stats['mean']:.3f}, Min: {format_stats['min']:.3f}, Max: {format_stats['max']:.3f}"
    )
    logger.info(
        f"Total Reward - Mean: {total_stats['mean']:.3f}, Min: {total_stats['min']:.3f}, Max: {total_stats['max']:.3f}"
    )
    ...
    for i in range(min(3, len(results.completion))):
        ...
        assistant_msg = next(
            (msg for msg in completion if msg["role"] == "assistant"), None
        )
        if assistant_msg:
