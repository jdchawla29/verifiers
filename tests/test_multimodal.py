...
from verifiers import Environment, SingleTurnEnv, Parser, Rubric
...
class MockProcessor:
    ...
    def __init__(self):
        ...
        self.tokenizer.apply_chat_template.side_effect = lambda messages, **kwargs: " ".join([f"{msg['role']}: {msg['content']}" for msg in messages])
...
class TestMultimodalUtils:
    ...
    def test_format_oai_chat_msg(self):
        ...
        assert formatted[0][0]["content"][1]["image_url"]["url"].startswith("data:image/png;base64,")
...
class TestMultimodalIntegration:
    ...
    async def test_multimodal_generation_flow(self, mock_openai_client):
        ...
        if "images" in test_input and test_input["images"]:
            ...
            assert isinstance(last_user_msg["content"], list), "Content should be a list for multimodal"
            ...
            image_items = [item for item in last_user_msg["content"] if item["type"] == "image_url"]
